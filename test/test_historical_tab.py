...
def test_format_dataframe(sample_dataframe):
    ...
    assert all(
        formatted_df["DATE"] == ["2021-12-01", "2023-06-01"]
...
def test_create_time_series(sample_dataframe):
    ...
    assert (
        len(fig.data) == len(sample_dataframe.columns) - 1
    ), "Incorrect number of traces in the plot"
...
def test_get_layout():
    ...
    assert any(
        isinstance(component, dcc.Graph) for component in layout.children
    ), "Graph component missing in layout"
    assert any(
        isinstance(component, dash_table.DataTable) for component in layout.children
    ), "DataTable component missing in layout"
