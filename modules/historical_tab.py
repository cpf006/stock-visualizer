...
def format_dataframe(df):
    ...
    df_transposed = df_transposed.applymap(lambda x: f"${x:.2f}" if isinstance(x, (int, float)) else x)
...
def get_layout():
    ...
    table = dash_table.DataTable(
        data=data,
        columns=columns,
        style_table={'overflowY': 'auto', 'height': '300px'},  # Enable vertical scrolling
        style_header={
